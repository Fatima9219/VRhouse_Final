...
public class HoverButtonVar : MonoBehaviour
{
    ...
    private void InvokeEvents(bool wasEngaged, bool isEngaged)
    {
        ...
        if (buttonDown && onButtonDown != null) {
            ...
            LogicStatusRegister.lastPTLQuotationUnit = transform.parent.name;
            if (transform.parent.name.Equals("PTL_1"))
                LogicStatusRegister.PTLIsQuoted[0] = true;
            if (transform.parent.name.Equals("PTL_2"))
                LogicStatusRegister.PTLIsQuoted[1] = true;
            if (transform.parent.name.Equals("PTL_3"))
                LogicStatusRegister.PTLIsQuoted[2] = true;
            if (transform.parent.name.Equals("PTL_4"))
                LogicStatusRegister.PTLIsQuoted[3] = true;
            if (transform.parent.name.Equals("PTL_5"))
                LogicStatusRegister.PTLIsQuoted[4] = true;
            if (transform.parent.name.Equals("PTL_6"))
                LogicStatusRegister.PTLIsQuoted[5] = true;
        }
